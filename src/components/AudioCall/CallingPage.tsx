...
const CallingPage = ({ navigation, route }: any) => {
    ...
    function findAnotherUser() {
        fetch(`http://192.168.0.103:3000/connectingToOther/${firstParticipantToken}`)
            ...
            .then(json => {
                if (json.Output[0].secondParticipantToken != null) {
                    navigation.push('RoomPage', { url: 'wss://audio-chat-nextjs-eqg35w5o.livekit.cloud/' + json.Output[0].roomName, token: json.Output[0].secondParticipantToken });
                } else {
                    navigation.push('RoomPage', { url: 'wss://audio-chat-nextjs-eqg35w5o.livekit.cloud/' + json.Output[0].roomName, token: json.Output[0].firstParticipantToken });
                }
            })
